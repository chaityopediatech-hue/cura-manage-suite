...
function AssistantPage() {
  ...
  const [messages, setMessages] = useState<Msg[]>([
    { role: "assistant", content: "Hello, I'm the MediCore assistant. I can help with general clinical questions and how to use the app. " + t("aiDisclaimer") },
  ]);
  ...
  const send = async () => {
    ...
    try {
      ...
      setMessages([...next, { role: "assistant", content: `I do not have enough information. (${msg})` }]);
  ...
  return (
    <div className="max-w-3xl mx-auto flex flex-col h-[calc(100vh-9rem)]">
      ...
      <div className="rounded-md border bg-warning/10 text-sm text-foreground/80 px-3 py-2 mb-3 flex gap-2">
      ...
      <div className="flex-1 rounded-lg border bg-card p-4 overflow-y-auto space-y-3">
        ...
        {busy && <div className="text-xs text-muted-foreground">{t("loading")}</div>}
        <div ref={endRef} />
      </div>

      <form onSubmit={(e) => { e.preventDefault(); send(); }} className="mt-3 flex gap-2">
        <Input placeholder={t("aiPlaceholder")} value={input} onChange={(e) => setInput(e.target.value)} disabled={busy} />
